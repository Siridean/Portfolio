...
def RunInstance(_d:dict):#Ticker:str, Interval:int, nbars:int):
    ...
    #Define the upper and lower limits for each gene
    Loss_lim = range(5, 20)
    ma_lim = range(1,101)
    ...
    #Lock limits using testing line below for easier debugging.
    ...
    #Get the Trading data from the broker
    Kstart, Kend, Kopen, Khigh, Klow, Kclose, Kvol =  LS.GetKline(Ticker=Ticker, Interval=Interval, nbars=nbars)

    def Update(instance, fitness:list, pbar:tqdm):
        """Update the progress bar and print the current best fitness to the terminal"""
        pbar.update(1)
        print(round(max(fitness),2), "%", sep="", end='\r')


    def fitness_function(genes, index):
        """The fitness function, which the program will try to optimise for."""

        #Crreate a strategy, and set default values
        Strat = LS.Strategy(0.04, 1+(genes[0]/1000), 1-(genes[1]/1000), Kstart, Kend, Kopen, Khigh, Klow, Kclose)
        
        #Create all of the indicators required by the strategy
        ...
        def ApplyStrat(t:int):
            """The Strategy Function that simulates opening and closing trading positions based on the critera specified in the function
            Use the Strategy class to open and close positions. The Strat class will automatically close positions based on take-profit or stop-loss conditions
    ...
    #Create a Genetic Optimisation instance and wrap it in a progress bar function.
    ...
    #Get key parameters such as best solution and it's corresponding fitness.
